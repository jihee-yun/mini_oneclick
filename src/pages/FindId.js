...
const FindId = () => {
  ...
  const onClickFindId = async () => {
    const lostIdGet = await AxiosApi.lostIdGet(inputName, inputEmail);

    if(lostIdGet.status === 200) {
      if(lostIdGet.data) {
        setModalOpen(true);
        setModalText("아이디 : " + lostIdGet.data);
      } else {
        setModalOpen(true);
  ...
  return(
    ...
    <Header />
      <Container>
        ...
        <div className="item">
          <button className="itemBtn" onClick={onClickFindId}>아이디 찾기</button>
          <Modal open={modalOpen} close={closeModal}>{modalText}</Modal>
        </div>
        <div className="findPw">
          <p>비밀번호가 기억나지 않으신가요?</p>
          <Link to='/findPw' className="link_style">비밀번호 찾기</Link>
        </div>
        <div className="login">
          <p>기존에 사용하던 계정이 있으신가요?</p>
          <Link to='/logiin' className="link_style">로그인</Link>
        </div>
      </Container>
      <Footer />
    </>
